...
handle_call({get_host_nodes}, _From, State) ->
    Result=try lib_host_server:get_host_nodes(State#state.specs_dir)  of
	       {ok,R1}->
		   {ok,R1};
	       {error,ErrorR}->
		   {error,["M:F [A]) with reason",lib_host_server,get_host_nodes,[State#state.specs_dir],"Reason=", ErrorR]}
	   catch
...
handle_call({get_application_config}, _From, State) ->
    Result=try lib_host_server:get_application_config(State#state.specs_dir)  of
	       {ok,R1}->
		   {ok,R1};
	       {error,ErrorR}->
		   {error,["M:F [A]) with reason",lib_host_server,get_host_nodes,[State#state.specs_dir],"Reason=", ErrorR]}
